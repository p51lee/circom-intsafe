...
use program_structure::expression_builders::{build_call, build_parallel_op, build_tuple};
...
use program_structure::statement_builders::{
    build_declaration, build_initialization_block, build_log_call,
};
...
pub fn apply_syntactic_sugar(program_archive: &mut ProgramArchive) -> Result<(), Report> {
    if program_archive.get_main_expression().is_anonymous_comp() {
        return Result::Err(anonymous_general_error(
            program_archive.get_main_expression().get_meta().clone(),
            "The main component cannot contain an anonymous call  ".to_string(),
        ));
    }
    ...
    for (_, t) in &mut program_archive.functions {
        ...
        if old_body.contains_anonymous_comp() {
            return Result::Err(anonymous_general_error(
                old_body.get_meta().clone(),
                "Functions cannot contain calls to anonymous templates".to_string(),
            ));
        } else {
...
fn check_anonymous_components_statement(stm: &Statement) -> Result<(), Report> {
    match stm {
        Statement::MultSubstitution {
            ...
            if lhe.contains_anonymous_comp() {
                Result::Err(anonymous_general_error(
                    meta.clone(),
                    "An anonymous component cannot be used in the left side of an assignment"
                        .to_string(),
                ))
            } else {
                if rhe.contains_anonymous_comp() && *op == AssignOp::AssignSignal {
                    let error =
                        format!("Anonymous components only admit the use of the operator <==");
            ...
            for arg in args {
                if let program_structure::ast::LogArgument::LogExp(exp) = arg {
                    if exp.contains_anonymous_comp() {
                        return Result::Err(anonymous_general_error(
                            meta.clone(),
                            "An anonymous component cannot be used inside a log".to_string(),
                        ));
                    }
                }
            }
        ...
        Statement::Assert { meta, arg } => {
            if arg.contains_anonymous_comp() {
                Result::Err(anonymous_general_error(
                    meta.clone(),
                    "An anonymous component cannot be used inside an assert".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Statement::Return { meta, value: arg } => {
            if arg.contains_anonymous_comp() {
                Result::Err(anonymous_general_error(
                    meta.clone(),
                    "An anonymous component cannot be used inside a function ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Statement::ConstraintEquality { meta, lhe, rhe } => {
            if lhe.contains_anonymous_comp() || rhe.contains_anonymous_comp() {
                Result::Err(anonymous_general_error(
                    meta.clone(),
                    "An anonymous component cannot be used with operator === ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Statement::Declaration {
            ...
            for exp in dimensions {
                if exp.contains_anonymous_comp() {
                    return Result::Err(anonymous_general_error(
                        meta.clone(),
                        "An anonymous component cannot be used to define a dimension of an array"
                            .to_string(),
                    ));
                }
            }
...
pub fn check_anonymous_components_expression(exp: &Expression) -> Result<(), Report> {
    use Expression::*;
    match exp {
        ...
        InfixOp { meta, lhe, rhe, .. } => {
            if lhe.contains_anonymous_comp() || rhe.contains_anonymous_comp() {
                Result::Err(anonymous_general_error(
                    meta.clone(),
                    "An anonymous component cannot be used in the middle of an operation "
                        .to_string(),
                ))
            } else {
                Result::Ok(())
        ...
        InlineSwitchOp {
                ...
                || if_false.contains_anonymous_comp()
            {
                Result::Err(anonymous_general_error(
                    meta.clone(),
                    "An anonymous component cannot be used inside an inline switch ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        ParallelOp { meta, rhe } => {
            if !rhe.is_call() && !rhe.is_anonymous_comp() && rhe.contains_anonymous_comp() {
                return Result::Err(anonymous_general_error(
                    meta.clone(),
                    "Bad use of parallel operator in combination with anonymous components "
                        .to_string(),
                ));
...
fn remove_anonymous_from_statement(
    ...
    match stm {
        Statement::MultSubstitution { meta, lhe, op, rhe } => {
            ...
            let subs = Statement::MultSubstitution {
                meta: meta.clone(),
                lhe: lhe,
                op: op,
                rhe: new_rhe,
        ...
        Statement::While { meta, cond, stmt } => {
            let id_var_while = "anon_var_".to_string()
                + &file_lib
                    .get_line(meta.start, meta.get_file_id())
                    .unwrap()
                    .to_string()
                + "_"
            ...
            let (body, comp_dec, mut var_dec, mut subs) = remove_anonymous_from_statement(
                templates,
                file_lib,
                *stmt,
            ...
            let b_while = if !comp_dec.is_empty() {
                var_declarations.push(build_declaration(
                    meta.clone(),
                    VariableType::Var,
                    id_var_while.clone(),
                    Vec::new(),
                ));
                subs.push(build_substitution(
                    meta.clone(),
                    id_var_while.clone(),
                    vec![],
                    AssignOp::AssignVar,
    ...
    match exp {
        AnonymousComp {
            ...
            if template.is_none() {
                return Result::Err(anonymous_general_error(
                    meta.clone(),
                    format!("The template {} does not exist", id),
                ));
            }
            ...
            if let Some(m) = names {
                // in case we have a list of names and assignments
                ...
                for (operator, name) in m {
                    ...
                    if inputs.contains_key(&name) {
                        ...
                        let error = format!(
                            "The template {} does not have an input signal named {}",
                            template.unwrap().get_name(),
                            name
                        );
                        return Result::Err(anonymous_general_error(meta.clone(), error));
                    }
                ...
                for value in signals {
                    inputs_to_assignments.insert(
                        inputs[n_expr].0.clone(),
                        (AssignOp::AssignConstraintSignal, value),
                    );
                    n_expr += 1;
                }
            ...
            for (name_signal, (operator, expr)) in inputs_to_assignments {
                ...
                let (mut declarations2, mut stmts, new_exp) =
                    remove_anonymous_from_expression(templates, file_lib, expr, var_access)?;
        ...
        Tuple { meta, values } => {
            ...
            Result::Ok((
                declarations,
                new_stmts,
                build_tuple(meta.clone(), new_values),
            ))
        }
        ParallelOp { meta, rhe } => {
        ...
        _ => Result::Ok((Vec::new(), Vec::new(), exp)),
    }
}
    ...
    match stm {
        ...
        Statement::IfThenElse {
            ...
            if cond.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used inside a condition ".to_string(),
                ))
            } else {
        ...
        Statement::Assert { meta, arg } => {
            if arg.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used in a return ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Statement::Return { meta, value: arg } => {
            if arg.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used inside a function ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Statement::ConstraintEquality { meta, lhe, rhe } => {
            if lhe.contains_tuple() || rhe.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used with the operator === ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Statement::Declaration {
            ...
            for exp in dimensions.clone() {
                if exp.contains_tuple() {
                    return Result::Err(tuple_general_error(
                        meta.clone(),
                        "A tuple cannot be used to define a dimension of an array ".to_string(),
                    ));
                }
            }
            Result::Ok(())
                ...
                match acc {
                    ArrayAccess(exp) => {
                        if exp.contains_tuple() {
                            return Result::Err(tuple_general_error(
                                meta.clone(),
                                "A tuple cannot be used to define a dimension of an array"
                                    .to_string(),
                            ));
                        }
                    }
    ...
    match exp {
        ...
        InfixOp { meta, lhe, rhe, .. } => {
            if lhe.contains_tuple() || rhe.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used in the middle of an operation".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        InlineSwitchOp {
            ...
            if cond.contains_tuple() || if_true.contains_tuple() || if_false.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used inside an inline switch".to_string(),
                ))
            } else {
                Result::Ok(())
            }
        ...
        Call { meta, args, .. } => {
            for value in args {
                if value.contains_tuple() {
                    return Result::Err(tuple_general_error(
                        meta.clone(),
                        "A tuple cannot be used as a parameter of a function call".to_string(),
                    ));
                }
            }
            Result::Ok(())
        ...
        ParallelOp { meta, rhe } => {
            if rhe.contains_tuple() {
                Result::Err(tuple_general_error(
                    meta.clone(),
                    "A tuple cannot be used in a parallel operator ".to_string(),
                ))
            } else {
                Result::Ok(())
            }
...
fn remove_tuples_from_statement(stm: Statement) -> Result<Statement, Report> {
    match stm {
            ...
            match (new_exp_lhe, new_exp_rhe) {
                (
                    ...
                    if values1.len() == values2.len() {
                        ...
                        while values1.len() > 0 {
                            ...
                            if let Expression::Variable { meta, name, access } = lhe {
                                ...
                                if name != "_" {
                                    ...
                                    substs.push(Statement::UnderscoreSubstitution {
                                        meta: meta,
                                        op,
                                        rhe: rhe,
                                    });
                        ...
                        return Result::Err(tuple_general_error(
                            meta.clone(),
                            "The number of elements in both tuples does not coincide".to_string(),
                        ));
                    } else {
                        return Result::Err(tuple_general_error(
                            meta.clone(),
                            "This expression must be in the right side of an assignment"
                                .to_string(),
                        ));
                    }
                }
                (lhe, rhe) => {
                    if lhe.is_tuple() || lhe.is_variable() {
                        return Result::Err(tuple_general_error(
                            rhe.get_meta().clone(),
                            "This expression must be a tuple or an anonymous component".to_string(),
                        ));
                    } else {
                        return Result::Err(tuple_general_error(
                            lhe.get_meta().clone(),
        ...
        Statement::IfThenElse {
            ...
            if else_case.is_none() {
                Result::Ok(Statement::IfThenElse {
                    meta: meta,
                    cond: cond,
                    if_case: b_if,
                    else_case: Option::None,
        ...
        Statement::InitializationBlock {
            ...
            Result::Ok(Statement::InitializationBlock {
                meta: meta,
                xtype: xtype,
                initializations: new_inits,
            })
        ...
        Statement::Substitution {
            ...
            if new_rhe.is_tuple() {
                return Result::Err(tuple_general_error(
                    meta.clone(),
                    "Left-side of the statement is not a tuple".to_string(),
                ));
            }
            if var != "_" {
                Result::Ok(Statement::Substitution {
                    meta: meta.clone(),
                    var: var,
                    access: access,
                    op: op,
                ...
                Result::Ok(Statement::UnderscoreSubstitution {
                    meta: meta,
                    op,
                    rhe: new_rhe,
                })
